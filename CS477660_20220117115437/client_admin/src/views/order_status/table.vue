<template>
	<el-main class="bg">
			...
			<el-row>
				...
				<el-col :xs="24" :sm="10" :lg="8">
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/order_status/table','add') || $check_action('/order_status/view','add')" class="el-button el-button--default el-button--primary" to="./view?">添加
						</router-link>
						<el-button v-if="user_group == '管理员' || $check_action('/order_status/table','del') || $check_action('/order_status/view','del')" class="" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			...
			<el-table-column prop="service_personal" label="服务人员"
				v-if="user_group == '管理员' || $check_field('get','service_personal')" min-width="200">
				<template slot-scope="scope">
					{{ get_user_service_personal(scope.row['service_personal']) }}
				</template>
			</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/order_status/table','set') || $check_action('/order_status/view','set') || $check_action('/order_status/view','get') || $check_action('//table','add') || $check_action('//view','add')" >
				...
				<template slot-scope="scope">
					...
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="openPayModal(scope.row)" v-if="scope.row.pay_state==='未支付' && (user_group == '管理员' || $check_pay('/order_status/table'))">
						<span>支付</span>
					</el-button>
				</template>
			</el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			...
			openPayModal(v){
				this.pay_obj.id = v.order_status_id
				this.payModalVisible = true
			},
			submitPay(){
				let _this = this;
				let url = this.$toUrl(this.query, "~/api/order_status/set?order_status_id="+this.pay_obj.id);
