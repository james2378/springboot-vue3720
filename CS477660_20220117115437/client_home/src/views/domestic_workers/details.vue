<template>
	<div class="diy_details page_domestic_workers" id="domestic_workers_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_domestic_workers'>
							<div_domestic_workers :obj="obj"></div_domestic_workers>
							<button class="btn btn-primary" v-if="$check_action('/domestic_workers/details','set')" @click="change_praise()" >点赞</button>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_domestic_workers from "@/components/diy/div_domestic_workers.vue";
	...
	export default {
		...
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "domestic_workers_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/domestic_workers/set?domestic_workers_id=' + obj["domestic_workers_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "domestic_workers",
							source_field: "domestic_workers_id",
							source_id: this.obj.domestic_workers_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.domestic_workers_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "domestic_workers",
					source_field: "domestic_workers_id",
					source_id: obj["domestic_workers_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/domestic_workers/set?domestic_workers_id=' + _this.obj["domestic_workers_id"], {
								praise_len
							}, (res) => {
								if(res.result){
